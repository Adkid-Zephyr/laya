...
[中文报告](README.zh-CN.md) · [Existing Feishu benchmark](../feishu_zh/README.md)
...
```bash
python research/benchmarks/chinese_intent_adaptation/audit.py
python -m unittest discover -s research/benchmarks/chinese_intent_adaptation -p 'test_*.py'
```
...
Hugging Face publication is pending repository-write authentication. The intended account/repository is `Adkid/laya-zh-intent-v3`; no claim of an available model download is made until the link is verified. The benchmark/results contribution can be reviewed independently.
